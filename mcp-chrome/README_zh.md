...
context，包含 `target`、`matchCount`、`nth`、`activeElement`、`selection` 和候选控件，`select` 和 `replace` 的事件级
`nth` 表示文本第 N 次出现，嵌套的 `target.nth` 表示第 N 个目标元素，两者都从 0 开始并可同时使用，
...
`NON_SERIALIZABLE_EVALUATE_RESULT`，并提示改为返回 `textContent`、`outerHTML` 等简单字段，evaluate 即使在全局
`inputMode=stealth` 时也默认使用 `precise`，其 `postCondition` 检查与主动作使用相同 evaluate mode，`postCondition` 不传时，
`success=true` 只表示脚本完成执行并返回；传入后如果超时仍不匹配，返回 `POST_CONDITION_FAILED` 和最后一次检查结果，
...
`url`、`status`、`timestamp`、`duration`，内联 network 结果会把单条 URL 限制为 2048 字符，截断时返回 `urlLength` 和
`urlTruncated: true`，显式 `output` 仍保存完整 URL，`urlPattern` 支持 `*` 匹配任意长度字符、`?` 匹配单个字符，`tabId`
...
input 和 evaluate 返回值追加 `actionExecuted`、`actionStatus`、`verificationStatus`、`failureStage` 和 `retryable`。验证状态分为 `matched`、`not_matched`、`unavailable`、`error`；debugger 超时且无法证明脚本已停止时，动作状态为 `unknown`。diagnostics 是附加能力，失败时返回 `diagnosticsStatus`，不会覆盖主动作结果。browse 和 wait 的主动作失败时，也会保留失败前已收集的 diagnostics。

`replace` 仅对 `textarea` 以及 `text`、`search`、`tel`、`url`、`password` 类型的 input 使用文本选区。其他 input 类型通过原生完整 value 更新，并返回请求值与浏览器规范化后的实际值。对不支持选区的类型单独执行 `select` 会返回 `UNSUPPORTED_SELECTION`。目标超时返回限量的 locator、tab、frame、匹配数和候选上下文。precise iframe evaluate 遇到失效执行上下文只重试一次，重复 URL 导致 frame 不唯一时拒绝猜测，响应包含 `retryAttempted`、限量的 `retryReason`，以及最终 Extension frame、parent frame、URL、CDP frame 和 execution context ID 组成的 `frameContext`。端口扫描会把普通的 WebSocket open 前失败聚合为 debug；已识别 MCP server 的认证或协议拒绝只在摘要变化时输出一条限量 warning。
