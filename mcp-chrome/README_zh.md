...
`NON_SERIALIZABLE_EVALUATE_RESULT`，并提示改为返回 `textContent`、`outerHTML` 等简单字段，`postCondition` 不传时，
...
`url`、`status`、`timestamp`、`duration`，`urlPattern` 支持 `*` 匹配任意长度字符、`?` 匹配单个字符，`tabId` 指定目标
Tab（Extension 模式），`frame` 不适用于日志，
