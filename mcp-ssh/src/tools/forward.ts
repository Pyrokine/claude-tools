...
const forwardLocalSchema = z.object({
                                        ...
                                        localHost: z.string().optional().describe('本地监听地址，默认 127.0.0.1'),
                                    })
...
const forwardRemoteSchema = z.object({
                                         ...
                                         remoteHost: z.string().optional().describe('远程监听地址，默认 127.0.0.1'),
                                     })
...
export function registerForwardTools(server: McpServer): void {
    server.registerTool('ssh_forward_local', {
...
效果：访问本地 localhost:8080 会转发到远程内网的 10.0.0.1:80`,
    ...
    server.registerTool('ssh_forward_remote', {
...
效果：远程访问 localhost:8080 会转发到本地的 127.0.0.1:3000`,
    ...
    server.registerTool('ssh_forward_close', {
        description: '关闭端口转发',
        inputSchema: forwardCloseSchema,
    }, (args) => handleForwardClose(args))

    server.registerTool('ssh_forward_list', {
