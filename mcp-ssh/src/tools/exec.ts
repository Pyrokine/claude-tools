...
async function handleExecAsUser(args: z.infer<typeof execAsUserSchema>) {
    try {
        const result = await sessionManager.execAsUser(
            args.alias,
            args.command,
            args.targetUser,
            {
...
async function handleExecSudo(args: z.infer<typeof execSudoSchema>) {
    try {
        const result = await sessionManager.execSudo(
            args.alias,
            args.command,
            args.sudoPassword,
            { timeout: args.timeout },
...
返回: stdout, stderr, exitCode, duration`,
    ...
    server.registerTool('ssh_exec_as_user', {
...
示例: ssh_exec_as_user(alias="server", command="whoami", targetUser="caros")`,
    ...
    server.registerTool('ssh_exec_sudo', {
