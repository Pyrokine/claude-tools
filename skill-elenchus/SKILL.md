...
| 输入          | 命令                                             |
|-------------|------------------------------------------------|
| 无参数         | 有暂存 → `git diff --cached`；否则 → `git diff HEAD` |
| "暂存区"       | `git diff --cached`                            |
| "工作区"       | `git diff`                                     |
...
同时执行 `git diff --stat`. 如果 diff 为空，告知用户并停止.
...
保存到输出目录 `cross-review.md`.

### 并行-3：合题——争议仲裁

收集所有被 CHALLENGED 的发现，逐个进入仲裁循环：

1. 派发 **opus** 仲裁 agent，给出 SUSTAINED / OVERTURNED 裁决
2. 如果裁决与多数意见矛盾，进入下一轮（新 opus agent 从零评估）
3. 连续 2 轮裁决不变 → 收敛；3 轮上限 → 标记 UNRESOLVED
