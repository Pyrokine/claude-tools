...
**默认必须显式输出中间过程**：只要是直接对用户执行 L1 串行模式，就必须把 `命题锚定` 和每一轮的 `正题 / 反题 / 合题` 按下方输出格式完整展示给用户，不能只返回合成结论，也不能静默省略中间轮次。只有被其他流程内部调用的引擎模式才允许静默。
...
- 仅在被其他流程内部调用时才允许静默运行，不向用户输出中间轮次
- 仅返回最终合题和关键推导路径
- 直接面对用户的 L1 串行模式不适用本条，必须显式展示中间过程
- 遵从调用方的轮次上限和聚焦范围
